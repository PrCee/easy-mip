...
class TelegramHandlerSimple:
    ...
    def handle_audio_message(self, chat_id: int, audio: Dict, session: Dict) -> str:
        """Processa mensagem de áudio (versão simplificada)"""
        ...
        try:
            ...
            # Versão simplificada - sem transcrição
            session["audio_path"] = file_path
            session["transcription"] = "Áudio recebido (transcrição não disponível)"
            session["state"] = "waiting_images"
            
            # Definir passos padrão
            session["steps"] = ["Passo 1", "Passo 2", "Passo 3"]
            
            return (f"✅ <b>Áudio recebido com sucesso!</b>\n\n"
                   f"📸 Agora envie as fotos dos passos (uma por vez).\n"
                   f"Você deve enviar {len(session['steps'])} fotos.")
    ...
    def generate_mip(self, chat_id: int, session: Dict) -> str:
        """Gera o MIP final (versão simplificada)"""
        try:
...
<html>
<head>
    <title>MIP - {session['title']}</title>
    <style>
        body {{ font-family: Arial, sans-serif; margin: 40px; }}
        .title {{ color: #00517C; font-size: 24px; text-align: center; }}
        .step {{ margin: 20px 0; }}
    </style>
</head>
<body>
    <div class="title">MIP - {session['title']}</div>
    <p>Gerado em: {datetime.now().strftime('%d/%m/%Y %H:%M')}</p>
    <p>Fotos recebidas: {len(session['images'])}</p>
    <p>Suporte: (11) 3090-0900</p>
</body>
</html>
"""
            ...
            # Enviar arquivo
            success = self.send_document(chat_id, html_path, "📄 MIP gerado com sucesso!")
            
            # Resetar sessão
            session["state"] = "initial"
            
            if success:
                return "✅ <b>MIP gerado com sucesso!</b>\n\nArquivo enviado.\n\nUse /new para criar outro MIP."
            else:
                return "❌ Erro ao enviar MIP. Tente novamente."
